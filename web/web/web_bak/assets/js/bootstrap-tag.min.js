...
!function(c){
    ...
    b.prototype = {
        ...
        show: function(){
            ...
            d.element.wrap(c('<div class="tags col-xs-11" style="overflow-x:scroll">')).parent().on("click", function(){
                d.textarea.focus()
            });
            if (d.values.length) {
            ...
            d.textarea = c('<textarea class="col-xs-12"/>').attr("placeholder", d.options.placeholder).insertAfter(d.element).on("focus", function(){
                    ...
                    if (c.trim(c(this).val()) && (!d.element.siblings(".typeahead").length || d.element.siblings(".typeahead").is(":hidden"))) {
                        ...
                        if (g.keyCode == 188) {
                            if (!d.element.siblings(".typeahead").length || d.element.siblings(".typeahead").is(":hidden")) {
                                ...
                                d.textarea.data("typeahead").select();
                                g.stopPropagation();
                                g.preventDefault()
                            }
                        }
            ...
            c(d.textarea.data("bs_typeahead").$menu).on("mousedown", function(){
                d.skip = true
            });
        ...
        add: function(g){
            ...
            if (!f.options.allowDuplicates) {
                ...
                if (e != -1) {
					...
					if(d.css('background-color').replace(/\s/g,'') == 'rgb(255,183,82)' || d.css('background-color').replace(/\s/g,'') == '#ffb752'){
						return;
					}
        ...
        process: function(){
            var d = c.grep(c.map(this.textarea.val().split(","), c.trim), function(f){
                return f.length > 0
            }), e = this;
            c.each(d, function(){
                e.add(this)
